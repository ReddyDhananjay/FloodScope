...
async function main() {
  ...
  try {
    const res = await fetch(`http://localhost:${process.env.PORT || 8000}/api/import/csv`, {
      method: 'POST',
    ...
    if (!res.ok) {
      // If server isn't running, import directly
      throw new Error('Server not running, use direct import');
    }
